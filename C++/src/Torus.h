...
public:
    int N1, N2, m;
    double T, rho, stabilityIdx = NULL;
    std::map<double, MatrixCircle> data;
    std::map<double, MatrixCircle> stableManifoldData, unstableManifoldData;
    bool torusInCollocation = false;
    bool torusConverged = false;

    Torus() : N1(NULL), N2(NULL), m(NULL) { };
...
/**
 * Subtraction operator between torus and periodic orbit solution.
