...
{
    int posAngle  = theta_2 * N2;
    Vector6d manifoldState = this->data[theta_1].row(posAngle);
    if (invert)
        manifoldState -= this->stableManifoldData[theta_1].row(posAngle);
    else
        manifoldState += this->stableManifoldData[theta_1].row(posAngle);
...
std::pair<std::complex<double>, Eigen::VectorXcd> Torus::findQPEigen(Matrix6d &monodromyMatrix)
{
    ...
    {
        if (std::abs(val) > 0.999 && std::abs(val) < 1.001 && std::arg(val) > 0)
        {
...
//        throw std::runtime_error("Torus sizes do not match!");
    ...
    Torus sumTorus = torus1;
    for (auto tor1dict = torus1.data.begin(), tor2dict = torus2.data.begin(); tor1dict != torus1.data.end(); tor1dict++, tor2dict++)
        sumTorus.data[tor1dict->first] = tor1dict->second + tor2dict->second;
    sumTorus.T += torus2.T;
    sumTorus.rho += torus2.rho;
    sumTorus.torusConverged = false;

    return sumTorus;
}

Torus operator*(double val, Torus &torus)
{
    for (auto tordict = torus.data.begin(); tordict != torus.data.end(); tordict++)
        tordict->second *= val;
    torus.T *= val;
    torus.rho *= val;
    torus.torusConverged = false;
