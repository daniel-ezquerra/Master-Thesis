...
void QPCollocationSolver::Solve(Torus &torus, Torus &prevTorus, Torus &tanTorus,
                                std::pair<double,double>config, double tol, int maxIter, bool getManifolds)
{
    ...
    while(error > tol)
    {
        ...
        errorPrev = error;
        error = correction.norm() / ((torus.N1 * (torus.m + 1) + 1) * torus.N2); // update error
        ...
        if (error > errorPrev)
            throw std::runtime_error("Solver not converging!");
    }
    // Update input torus
    ...
    if (getManifolds)
    {
        ...
        solver.analyzePattern(matrixB);   // since the pattern of jMatrix is always the same, analyze only once
        solver.factorize(matrixB);
...
void QPCollocationSolver::arrayToTorus(Eigen::VectorXd &array, Torus &torus)
{
    ...
    torus.T = array[dimVector];
    torus.rho = array[dimVector + 1];
}
...
void QPCollocationSolver::findManifolds(Torus &torus, SparseMatrix &stateTransitionMatrices)
{
    ...
    // Find the stable and unstable manifold vector
    int count = 0;
    double firstUnstableManifoldValue, firstStableManifoldValue;
    Eigen::VectorXd firstUnstableManifoldVector(dimBlock), firstStableManifoldVector(dimBlock);
    for (const auto &eigenval :eigenValues)
    {
        if (eigenval.imag() == 0 && eigenval.real() > 1.5) {
            firstUnstableManifoldValue = eigenval.real();
            firstUnstableManifoldVector = eigenVectors.col(count).real();
        }
        if (eigenval.imag() == 0 && eigenval.real() > 0 && eigenval.real() < 0.5) {
            firstStableManifoldValue = eigenval.real();
            firstStableManifoldVector = eigenVectors.col(count).real();
        }
        count++;
    }

    // Save stability index
    torus.stabilityIdx = (firstUnstableManifoldValue + firstStableManifoldValue) / 2;

    // Propagate the manifold eigenvector
    Eigen::VectorXd stableManifoldVector(dimBlock);
    Eigen::VectorXd unstableManifoldVector(dimBlock);
    MatrixCircle stableManifoldMatrix(N2, dimState);
    MatrixCircle unstableManifoldMatrix(N2, dimState);
    for (int i = 0; i < N1 * (m + 1) + 1; i++)
    {
        double time = collocationArray[i];

        if (i == 0)    // do not propagate first and last eigenvectors
        {
            stableManifoldVector = firstStableManifoldVector;
            unstableManifoldVector = firstUnstableManifoldVector;
        }
        else
        {                               // remove scaling produced by state transition matrix
            stableManifoldVector = pow(firstStableManifoldValue, -time) *
                                   stateTransitionMatrices.block((i-1) * dimBlock, 0, dimBlock, dimBlock) * firstStableManifoldVector;
            unstableManifoldVector = pow(firstUnstableManifoldValue, -time) *
                                   stateTransitionMatrices.block((i-1) * dimBlock, 0, dimBlock, dimBlock) * firstUnstableManifoldVector;
        }

        // Transform vector into matrix form
        for (int j = 0; j < N2; j++)
        {
            stableManifoldMatrix.row(j) = stableManifoldVector.segment(j*dimState, dimState);
            unstableManifoldMatrix.row(j) = unstableManifoldVector.segment(j*dimState, dimState);
        }

        torus.stableManifoldData[time] = stableManifoldMatrix;  // fill torus information
        torus.unstableManifoldData[time] = unstableManifoldMatrix;
    }
}
