...
void POCollocationSolver::Solve(PeriodicOrbit &orbit, PeriodicOrbit &prevOrbit, double Href, double tol, int maxIter, bool getManifolds, const PeriodicOrbit &tanOrbit)
{
    // Transform to array
    Eigen::VectorXd orbitV = orbitToArray(orbit);
    Eigen::VectorXd prevOrbitV = orbitToArray(prevOrbit);
    Eigen::VectorXd tanOrbitV(prevOrbitV.size()); tanOrbitV.setConstant(NULL);
    if (tanOrbit.N1 != NULL)
        tanOrbitV = orbitToArray(tanOrbit);
    ...
    {
        // Fill fArray
        fArray = getCollocationSyst(orbitV, prevOrbitV, tanOrbitV, orbit, Href);
        // Fill jMatrix (Jacobian)
        jMatrix = getCollocationJacobian(orbitV, prevOrbitV, tanOrbitV, orbit, Href);
    ...
    if (getManifolds)
    {
        ...
        int dimState = 6, dimMatrix = orbit.N1 * (orbit.m + 1) * dimState;
        SparseMatrix matrixA = SparseMatrix(jMatrix.block(0, 0, dimMatrix, dimState));
        SparseMatrix matrixB = SparseMatrix(-jMatrix.block(0, dimState, dimMatrix, dimMatrix));
        solver.compute(matrixB);
        SparseMatrix stateTransitionMatrices = solver.solve(matrixA);

        // Fill manifold information
        findManifolds(orbit, stateTransitionMatrices);
        std::cout << "Manifolds computed!" << std::endl;
    }
}


void POCollocationSolver::findManifolds(PeriodicOrbit &orbit, SparseMatrix &stateTransitionMatrices)
{
    int dimState = 6, N1 = orbit.N1, m = orbit.m;
    int dimMatrix = N1 * (m + 1) * dimState, dimBlock = dimState;
    Eigen::VectorXd collocationArray = tools3BP::gaussLegendreCollocationArray(N1, m);

    // Retrieve monodromy matrix
    ...
    for (int i = 0; i < N1 * (m + 1) + 1; i++)
    {
        ...
        orbit.manifoldData[time] = 1E-5 * manifoldVector;  // fill torus information    // TODO: Do not used hard-coded value
    }
}
                                                        ...
                                                        double Href)
{
    ...
    for (const auto &time :timeArray)
    {
        ...
        Eigen::VectorXd tanState = tanOrbitV.segment(numTime * dimState, dimState);
        ...
        // Energy condition
        if (tanOrbitV[0] == NULL)
            extraCondition += getJacobi(state);
        // Arc-length condition
        else
            extraCondition += (state - prevState).transpose() * tanState;
    ...
    fArray[dimVector+1] = extraCondition / (N1 * (m + 1) + 1) - Href;
                                                         ...
                                                         double Href)
{
    ...
    for (const auto &time :timeArray)
    {
        ...
        // Arc-length condition
            ...
            fillSpBlock<Eigen::RowVectorXd>(tripletList, tanState.transpose() / (N1 * (m + 1) + 1),
                                            dimVector + 1, numTime * dimState, 1, dimState);
        }
...
Eigen::VectorXd POCollocationSolver::orbitToArray(const PeriodicOrbit &orbit)
{
...
void POCollocationSolver::arrayToOrbit(Eigen::VectorXd &array, PeriodicOrbit &orbit)
{
    ...
    orbit.T = array[(N1 * (m + 1) + 1) * dimState];
}
...
Eigen::Vector3d POCollocationSolver::getLagrangePoint(lagrangePoint &point) // TODO: Change intial guess
{
    ...
    try
    {
        xVal = newton_raphson_iterate(rootFun, guess, bound.first, bound.second, get_digits, maxit);
    }
    catch (std::exception& e)
    {
        std::cout << "Error when finding Lagrange points, try changing initial guess.\n" << e.what() << std::endl;
    }
...
PeriodicOrbit POCollocationSolver::getLinearApproximation(lagrangePoint point, periodicOrbit orbit, int N1, int m, double H, std::pair<double, double>parSpan)
{
    Eigen::Vector3d lPosition = lagrangePosition[point];
    Vector6d stateLagrange; stateLagrange.setZero(); stateLagrange.segment(0,3) = lPosition;
    if (H > getJacobi(stateLagrange)) {
        std::cout << "Max. energy: " + std::to_string(getJacobi(stateLagrange)) << std::endl;
        throw std::runtime_error("Invalid energy.");
    }

    if (point == L3 && orbit == planar)
    {
        double auxVar = parSpan.first;
        parSpan.first = -parSpan.second;
        parSpan.second = -auxVar;
    }

    if (orbit == planar)
        return planarLinearApproximation(N1, m, H, point, parSpan);
    else
        return threeDLinearApproximation(N1, m, H, point, orbit, parSpan);
}
...
PeriodicOrbit POCollocationSolver::planarLinearApproximation(int N1, int m, double Href, lagrangePoint point,
                                                             std::pair<double, double>x0Span)
{
    ...
    // Find initial conditions that verify energy requirement
    auto energyFun = std::bind(&POCollocationSolver::energyConditionPlanar, this, std::placeholders::_1, Href, point);
    std::pair<double, double> result = bisect(energyFun, x0Span.first ,x0Span.second, TerminationCondition());
    double x0 = (result.first + result.second) / 2;

    for (const auto &theta : thetaArray)
        periodicOrbitApproximation[theta] = planarStateApproximation(2 * M_PI * theta, x0, point);

    double s = planarStateApproximation(-M_PI_2, 1, point)[3];  // Retrieve s from dx value
    double T = 2 * M_PI / s;
    return PeriodicOrbit(periodicOrbitApproximation, T, N1, m);
}


PeriodicOrbit POCollocationSolver::threeDLinearApproximation(int N1, int m, double Href, lagrangePoint point,
                                                             periodicOrbit orbit, std::pair<double, double>AzSpan)
{
    stateDict periodicOrbitApproximation;   // allocate solution
    Eigen::VectorXd thetaArray = tools3BP::gaussLegendreCollocationArray(N1, m);
    ...
    std::pair<double, double> result = bisect(energyFun, AzSpan.first, AzSpan.second, TerminationCondition());
    double Az = (result.first + result.second) / 2;

    // Create linear approximation of 3D orbit
    for (const auto &theta :thetaArray)
        periodicOrbitApproximation[theta] = threeDStateApproximation(2 * M_PI * theta, Az, point, orbit);

    // Define period of the orbit
    double T = threeDStateApproximation(0, Az, point, orbit, true)[0];
    if (orbit == vertical)
        T *= 2;
...
double POCollocationSolver::energyConditionPlanar(const double &x, double Href, lagrangePoint point)
{
    Vector6d state = planarStateApproximation(0, x, point);
...
{
    Vector6d state = threeDStateApproximation(0, Az, point, orbit);
    return getJacobi(state) - Href;
}


Vector6d POCollocationSolver::planarStateApproximation(double theta, double x0, lagrangePoint point)
{
    double u = massParameter;
    Eigen::Vector3d lPosition = lagrangePosition[point];
    double distLagrange = lPosition[0];
    double K = (1 - u) / pow(pow(u + distLagrange, 2), 3 / 2.) + u / pow(pow(1 - u - distLagrange, 2), 3 / 2.);
    double alpha = 1 - K / 2;
    double beta = sqrt((2 * K + 1) * (K - 1));
    double s = sqrt(alpha + sqrt(alpha * alpha + beta * beta));
    double v = (s * s + 2 * K + 1) / (2 * s);

    double x = x0 * cos(theta);
    double y = -v * x0 * sin(theta);
    double dx = -x0 * s * sin(theta);
    double dy = -v * s * x0 * cos(theta);

    Vector6d state = Vector6d(x, y, 0, dx, dy, 0);
    state.segment(0, 3) += lPosition;
    return state;
}


Vector6d POCollocationSolver::threeDStateApproximation(double theta, double Az, lagrangePoint point, periodicOrbit orbit, bool returnPeriod) {
    double u = massParameter;
    Eigen::Vector3d lPosition = lagrangePosition[point];
    double distLagrange = lPosition[0];
    double gamma = abs((1 - u) - distLagrange);
    double factPoint;

    double c_2, c_3, c_4;
    if (point == L1) {
        c_2 = (u + (1 - u) * pow(gamma, 3) / pow(1 - gamma, 3)) / pow(gamma, 3);
        c_3 = (u - (1 - u) * pow(gamma, 4) / pow(1 - gamma, 4)) / pow(gamma, 3);
        c_4 = (u + (1 - u) * pow(gamma, 5) / pow(1 - gamma, 5)) / pow(gamma, 3);
    } else if (point == L2) {
        c_2 = (u + (1 - u) * pow(gamma, 3) / pow(1 + gamma, 3)) / pow(gamma, 3);
        c_3 = (-u - (1 - u) * pow(gamma, 4) / pow(1 + gamma, 4)) / pow(gamma, 3);
        c_4 = (u + (1 - u) * pow(gamma, 5) / pow(1 + gamma, 5)) / pow(gamma, 3);
    } else {
        c_2 = (1 - u + u * pow(gamma, 3) / pow(1 + gamma, 3)) / pow(gamma, 3);
        c_3 = (1 - u + u * pow(gamma, 4) / pow(1 + gamma, 4)) / pow(gamma, 3);
        c_4 = (1 - u + u * pow(gamma, 5) / pow(1 + gamma, 5)) / pow(gamma, 3);
    }
    ...
    double a_31 = -9 * lmda / (4 * d_2) * (4 * c_3 * (k * a_23 - b_21) + k * c_4 * (4 + pow(k, 2))) +
                  (9 * pow(lmda, 2) + 1 - c_2) / (2 * d_2) *
                  (3 * c_3 * (2 * a_23 - k * b_21) + c_4 * (2 + 3 * pow(k, 2)));
    ...
    int r;
    if ((orbit == northHalo && point == L1) || (orbit == southHalo && point == L2))
        r = 1;
    else
        r = 3;
    int d_r = 2 - r;
    ...
    if (returnPeriod)
        return Vector6d(2 * M_PI / (w * lmda), 0, 0, 0, 0, 0);
