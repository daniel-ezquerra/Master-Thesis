...
if writing:
    f = open("periodicOrbit.dat", "w")
    for num, time in enumerate(solData.t):
        f.write("{:.4f}".format(time) + " " +  "{:.16f}".format(solData.y[0,num]) + " " +  "{:.16f}".format(solData.y[1,num])+ " " +  "{:.16f}".format(solData.y[2,num])+ " " +  "{:.16f}".format(solData.y[3,num])+ " " +  "{:.16f}".format(solData.y[4,num])+ " " +  "{:.16f}".format(solData.y[5,num])+ "\n")
    f.close()
...
def getCollocationSystem( torus, var, prevTorus, prevVar, tanTorus, tanVar, consts, N1, N2, m ):
    ...
    for time in torus.keys():
        ...
        for numState, state in enumerate(torus[time]):
            indexState = dimState * numState
            dxdth2Section = dxdth2[indexState:indexState+dimState]
            dxdth1Section = Tprev * modelDynamicsCR3BPExp(prevTorus[time][numState], dxdth2Section, lambda1Prev, lambda2Prev) - rhoPrev * dxdth2Section
...
while error > tol:
    ...
    correction = spsolve(jacobian, fArray)
    error = np.linalg.norm(correction)
    count += 1
    print("Iteration no: ", count, " | Error: ", error)
    newGuess = guessSolFlat - correction
    var = newGuess[-6:]
